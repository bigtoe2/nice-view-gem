...
static void set_peripheral_output_status(struct zmk_widget_screen *widget,
    ...
    LOG_DBG("Peripheral connection changed: %s", state->connected ? "connected" : "disconnected");                                            
...
static struct peripheral_status_state connection_status_get_state(const zmk_event_t *eh) {
...
ZMK_DISPLAY_WIDGET_LISTENER(widget_connection_status, struct peripheral_status_state,
...
int zmk_widget_screen_init(struct zmk_widget_screen *widget, lv_obj_t *parent) {
    ...
    lv_canvas_set_buffer(top, widget->cbuf2, 2*BUFFER_SIZE, BUFFER_SIZE, LV_IMG_CF_TRUE_COLOR);
